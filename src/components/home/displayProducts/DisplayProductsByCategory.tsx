...
const DisplayProductsByCategory = ({ category }: Props) => {
  return (
    <div id="products-category-display-container" className="p-4 md:p-6 lg:p-9">
      <h1 className="text-center w-full text-xl lg:text-2xl font-semibold capitalize my-6">{category.section}</h1>
      <div id="products-wrapper" className="grid lg:grid-cols-4  grid-cols-2 gap-6   ">
        {category.items.map((item, index) => (
          <motion.div key={index} whileHover={{scale:1.05}} id="product-item" className=" relative cursor-pointer">
            <div  id="product-image " className="pt-[150%]  relative  " >
              <Image src={item.image} alt="" fill  className="rounded-xl shadow-lg w-full h-full object-cover absolute" />
